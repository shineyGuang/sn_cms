...
from app.extension import ResponseMessage
...
def pbkdf2_hex(
    ...
    .. deprecated:: 2.0
        Will be removed in Werkzeug 2.basic. Use :func:`hashlib.pbkdf2_hmac`
        instead.
    ...
    warnings.warn(
        ...
        " 2.basic. Use 'hashlib.pbkdf2_hmac().hex()' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def pbkdf2_bin(
    ...
    warnings.warn(
        ...
        " 2.basic. Use 'hashlib.pbkdf2_hmac()' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def safe_str_cmp(a: str, b: str) -> bool:
    ...
    .. deprecated:: 2.0
        Will be removed in Werkzeug 2.basic. Use
    ...
    warnings.warn(
        ...
        " 2.basic. Use 'hmac.compare_digest' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def create_access_token(
        subject: t.Union[str, t.Any], expires_delta: timedelta = None
) -> str:
    if expires_delta:
...
reusable_oauth2 = OAuth2PasswordBearer(
    tokenUrl=f'{configs.API_V1_STR}/login/access-token'
)
...
def get_current_user(token: str = Depends(reusable_oauth2)):
    try:
        payload = jwt.decode(
            token.replace("Bearer ", ""), configs.SECRET_KEY, algorithms=[configs.ALGORITHM]
        )
        auth_user_id = json.loads(payload.get("sub")).get("auth_user_id")
        cur_user = UserInfo.filter(auth_user_id=auth_user_id)
        if not cur_user:
...
if __name__ == '__main__':
    res = generate_password_hash("Zcg@123456")
    print(res, len(res))
